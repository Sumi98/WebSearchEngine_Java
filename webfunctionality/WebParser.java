...
public class WebParser {
	...
	public static void generate_Files_List() {
		try {
	...
	public static List<String> parse(String doc) throws IOException {
		...
		for (String token : tokens)
			tokenList.add(token);
		remove_Stop_Words(tokenList);
		return tokenList;
	}

	// removing the stop words from the tokens
	public static void remove_Stop_Words(List<String> inputList) {
		inputList.removeIf(ip -> stopWords.contains(ip));
	}

	public static void save_Doc(Document doc) {
		try {
